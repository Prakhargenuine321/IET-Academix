...
const AdminPYQUpload = ({ onSubmit, loading, initialData, isEdit, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(isEdit ? "Failed to update PYQ." : "Failed to upload PYQ.");
    }
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-4 bg-white dark:bg-gray-800 p-6 rounded-xl shadow">
      <h2 className="text-xl font-semibold mb-2">{isEdit ? "Edit PYQ" : "Upload PYQ"}</h2>
      ...
      <div>
        <label className="block mb-1 font-medium">PYQ File URL*</label>
        <input
          type="url"
          name="fileUrl"
          value={form.fileUrl}
